{ config, ... }:
{
  customLib.math =
    let
      ...
      unitsLib = config.customLib.units;
      inherit (mathLib)
        mod
        floor
        isEven
    ...
    rec {
      ...
      modExp =
        ...
        let
          ...
          compute =
            ...
            in
            if bit == 1 then mod (squared * base) m else squared;
        in
      ...
      rsaKeypair =
        ...
        let
          ...
          checkPrime = n: if isPrime n then n else throw "rsaKeypair: ${toString n} is not prime!";
