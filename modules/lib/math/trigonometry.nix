    ...
    let
      ...
      sin =
        ...
        let
          ...
          evalQ0 = if offLeqPi4 then sinOff else cosHalfOff;

          # Evaluation for quadrant 1.
          evalQ1 = if offLeqPi4 then cosOff else sinHalfOff;
        in
        if norm.quadrant == 0 then
          evalQ0
        # Evaluation for quadrant 2 and 3 reuse evalQ0 / evalQ1 with sign flips.
        else if norm.quadrant == 1 then
          evalQ1
        else if norm.quadrant == 2 then
          -evalQ0
        else
          -evalQ1;
      ...
      tan =
        ...
        let
          cosX = cos x;
        in
        if cosX == 0.0 then (throw "`cos x` cannot equal 0!") else (sin x) / cosX;
    in
    {
      inherit sin cos tan;
    };
