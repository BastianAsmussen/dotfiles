...
in {
  flake.nixosModules.extra_hjem = {
    ...
    inherit (lib) mkOption concatLines mapAttrsToList getExe;
  ...
  in {
    options.home.programs.hyprland = {
      ...
      extraConfig = mkOption {
        description = "Hyprland configuration string.";
        default = "";
      };
    ...
    config = lib.mkIf cfg.enable {
      hjem.users.${username}.files.".config/hypr/hyprland.conf".text = cfg.finalConfig;
      home.programs.hyprland = {
        finalConfig = (toHyprconf {attrs = cfg.settings;}) + cfg.extraConfig;
        extraConfig = let
  ...
  flake.lib.generators.toHyprconf = {
    ...
    inherit (lib) all concatMapStringsSep concatStrings concatStringsSep filterAttrs foldl generators hasPrefix isAttrs isList mapAttrsToList replicate;
