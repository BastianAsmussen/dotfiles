  ...
  flake.nixosModules.pia =
    ...
    let
      ...
      connectScript = pkgs.writeShellScript "pia-connect" ''
        ...
        WG_HOSTNAME="$(printf '%s' "$SERVERS" | ${jq} -r ".regions[] | select(.id == \"$REGION\") | .servers.wg[0].cn")"
        WG_IP="$(printf '%s' "$SERVERS" | ${jq} -r ".regions[] | select(.id == \"$REGION\") | .servers.wg[0].ip")"
        ...
        WG_STATUS="$(printf '%s' "$WG_INFO" | ${jq} -r '.status')"
        [ "$WG_STATUS" = "OK" ] || { echo "addKey failed: $WG_INFO" >&2; exit 1; }

        PEER_IP="$(printf '%s' "$WG_INFO" | ${jq} -r '.peer_ip')"
        SERVER_KEY="$(printf '%s' "$WG_INFO" | ${jq} -r '.server_key')"
        SERVER_PORT="$(printf '%s' "$WG_INFO" | ${jq} -r '.server_port')"
        VPN_GW="$(printf '%s' "$WG_INFO" | ${jq} -r '.server_vip')"
        VPN_DNS="$(printf '%s' "$WG_INFO" | ${jq} -r '.dns_servers[0] // empty')"
        ...
        if [ -n "$VPN_DNS" ]; then
          ${ip} route add "$VPN_DNS/32" dev "$INTERFACE" 2>/dev/null || true
          ${resolvectl} dns "$INTERFACE" "$VPN_DNS"
        ...
        PF_RESPONSE="$(${curl} -sG --max-time 30 \
          --interface "$INTERFACE" \
          --connect-to "$WG_HOSTNAME::$VPN_GW:" \
          --cacert "$CA_CERT" \
          ...
          --interface "$INTERFACE" \
          --insecure \
          "https://$VPN_GW:19999/getSignature" \
          --data-urlencode "token=$TOKEN")"
        PF_STATUS="$(printf '%s' "$PF_RESPONSE" | ${jq} -r '.status')"
        [ "$PF_STATUS" = "OK" ] || { echo "getSignature failed: $PF_RESPONSE" >&2; exit 1; }

        PF_PAYLOAD="$(printf '%s' "$PF_RESPONSE" | ${jq} -r '.payload')"
        PF_SIGNATURE="$(printf '%s' "$PF_RESPONSE" | ${jq} -r '.signature')"
        PF_PORT="$(printf '%s' "$PF_PAYLOAD" | base64 -d | ${jq} -r '.port')"
        ...
        BIND_RESPONSE="$(${curl} -sG --max-time 30 \
          --interface "$INTERFACE" \
          --insecure \
          "https://$VPN_GW:19999/bindPort" \
          --data-urlencode "payload=$PF_PAYLOAD" \
          --data-urlencode "signature=$PF_SIGNATURE")"
        BIND_STATUS="$(printf '%s' "$BIND_RESPONSE" | ${jq} -r '.status')"
        [ "$BIND_STATUS" = "OK" ] || { echo "bindPort failed: $BIND_RESPONSE" >&2; exit 1; }
      ...
      disconnectScript = pkgs.writeShellScript "pia-disconnect" ''
        ...
        if [ -f /run/pia/dns ]; then
          VPN_DNS="$(cat /run/pia/dns)"
          ${ip} route del "$VPN_DNS/32" dev "$INTERFACE" 2>/dev/null || true
        fi
        ...
        ${ip} -6 rule del uidrange "$QBT_UID-$QBT_UID" unreachable priority 100 2>/dev/null || true
        ${ip} link del dev "$INTERFACE" 2>/dev/null || true
        echo "[pia-connect] Interface $INTERFACE removed"
      '';
    ...
    in
    {
      ...
      config = mkIf cfg.enable {
        ...
        # Stub resolver at 127.0.0.53 so qBittorrent DNS queries go via loopback
        # (exempt from the kill switch) rather than directly to the home router,
        # which is unreachable through PIA's tunnel.
