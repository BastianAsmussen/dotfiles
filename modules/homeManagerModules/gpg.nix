{
  flake.homeModules.gpg =
    ...
    let
      ...
      getAllKeys =
        keyDir: builtins.filter (x: lib.strings.hasSuffix ".asc" x) (attrNames (readDir keyDir));
      isKeyAllowed = keyName: disallowedKeys: !elem keyName disallowedKeys;
      filterAllowedKeys = allKeys: disallowedKeys: filter (key: isKeyAllowed key disallowedKeys) allKeys;

      # Map keys to their paths and trust levels.
      mapKeyPaths =
        keys: keyDir: trustMap: defaultTrust:
        map (keyName: {
          source = "${keyDir}/${keyName}";
          trust = if hasAttr keyName trustMap then trustMap.${keyName} else defaultTrust;
        }) keys;
      ...
      publicKeys =
        mapKeyPaths (filterAllowedKeys (getAllKeys keyDir) cfg.disallowedKeys) keyDir cfg.keyTrustMap
          cfg.trustLevel;
    in
    {
      options.gpg = {
      ...
      config = mkIf cfg.enable {
        warnings = concatLists [
          ...
          (
            if length missingKeys.trustMap > 0 then
              [
                ''
