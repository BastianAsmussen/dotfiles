{ config, ... }:
{
  perSystem =
    { pkgs, lib, ... }:
    let
      inherit (config.flake.lib.custom.units)
        mibToBytes
        rateToKiBps
        durationToSeconds
        toBinary
      ...
      cases = mkCases (mibCases ++ rateCases ++ durationCases ++ toBinaryCases ++ pow2Cases);
      results = lib.runTests cases;
    in
    {
      checks.tests-units =
        if results == [ ] then
          pkgs.runCommandLocal "tests-units-pass" { } "touch $out"
        else
          pkgs.runCommandLocal "tests-units-fail"
            {
